'''
This is a program for handling large deformations. 
It is recommended to lower the learning rate by 0.00001~0.0001
'''

import os
import torch
import numpy as np
import random
import sys
import math
from scipy.io import loadmat
sys.path.append("./layers")
sys.path.append("./utils")

from criterion import criterion_warmup, criterion_train
from EarlyStop import EarlyStopping
from FCNN import DNN
from utils import save_checkpoint
from readData import pyramidDataset, collate_fn
from result_plot import to_matlab, to_txt

from configs.config import config
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
# device = torch.device('cpu')

def count_parameters(model):
    '''Calculate the memory size of the model'''
    return sum(p.numel() for p in model.parameters() if p.requires_grad)

def seed_everything(seed_value):
    random.seed(seed_value)
    np.random.seed(seed_value)
    torch.manual_seed(seed_value)
    os.environ['PYTHONHASHSEED'] = str(seed_value)
    
    if torch.cuda.is_available(): 
        torch.cuda.manual_seed(seed_value)
        torch.cuda.manual_seed_all(seed_value)
        torch.backends.cudnn.deterministic = True
        torch.backends.cudnn.benchmark = True

seed_everything(1234)
model = DNN(config['layers']).to(device)
max_iter=config['max_iter']; max_eval = int(1.25 * max_iter)

optimizer_lbfgs = torch.optim.LBFGS(
    model.parameters(), lr=1, max_iter=max_iter, max_eval=max_eval,
    history_size=50, tolerance_grad=1e-06,
    tolerance_change=5e-06,
    line_search_fn="strong_wolfe")

optimizer_adam = torch.optim.Adam(
    model.parameters(), lr=config['warm_lr'],  eps=1e-8)

early_stop_adam = EarlyStopping(
        patience=config['patience_adam'], delta=config['delta_warm_adam'], 
        path='./weights/checkpoint/checkpoint_adam.pth')
early_stop_lbfgs = EarlyStopping(
        patience=config['patience_lbfgs'], delta=config['delta_warm_lbfgs'], 
        path='./weights/checkpoint/checkpoint_lbfgs.pth')

img_dataset = pyramidDataset(config['data_path'])
# RG, ROI, XY, XY_roi, Ixy = img_dataset.data_collect(device)
train_loader = torch.utils.data.DataLoader(
    img_dataset, batch_size=config['Batchframes'], 
    shuffle=False, collate_fn=collate_fn)

RGL={}; ROIL={}; XYL={}; XY_roiL={}; IxyL={}
SCALE_FACTOR_LIST = [4, 2, 1]
for SCALE_FACTOR in SCALE_FACTOR_LIST:
    ref_img, roi_img, XYi, XY_roii, Ixyi = img_dataset.data_collect(SCALE_FACTOR, device)
    RGL[f'{SCALE_FACTOR}'] = ref_img; ROIL[f'{SCALE_FACTOR}'] = roi_img
    XYL[f'{SCALE_FACTOR}'] = XYi; XY_roiL[f'{SCALE_FACTOR}'] = XY_roii
    IxyL[f'{SCALE_FACTOR}'] = Ixyi

print(f"Simple FCN has {count_parameters(model):,} trainable parameters")

if not os.path.exists(config['data_path']+'scale_information/'+'SCALE.mat'):
    raise ValueError("please run scale_list.py firstly")
else:
    SCALE = loadmat(config['data_path']+'scale_information/'+'SCALE.mat')

def closure1(model, Ixy, XY_roi, XY, Iref, Idef, ROI, scale):
    optimizer_lbfgs.zero_grad()
    UV = model(Ixy)
    loss, mae = criterion_warmup(UV, XY_roi, XY, Iref, Idef, ROI, scale)
    loss.backward()
    config['epoch'] += 1
    early_stop_lbfgs(mae, model, optimizer_lbfgs)
    return loss, mae

def closure2(model, Ixy, XY_roi, XY, Iref, Idef, ROI, scale):
    optimizer_lbfgs.zero_grad()
    UV = model(Ixy)
    loss, mae = criterion_train(UV, XY_roi, XY, Iref, Idef, ROI, scale)
    loss.backward()
    config['epoch'] += 1
    early_stop_lbfgs(mae, model, optimizer_lbfgs)
    return loss, mae

def warm_up(i, Ixy, XY_roi, XY, RG, DG, ROI, scale_factor):
    if config['warm_adam_epoch'] != 0:
        print("warm adam start:")
        for iter in range(config['warm_adam_epoch']):
            optimizer_adam.zero_grad()
            UV = model(Ixy)
            loss, mae = criterion_warmup(
                UV, XY_roi, XY, RG, DG, ROI, SCALE['scale'][i]/scale_factor
                )
            loss.backward()
            optimizer_adam.step()
            config['epoch'] += 1
            early_stop_adam(mae, model, optimizer_adam)
            if config['epoch']%config['print_feq'] == 1:
                epoch =  config['epoch'] 
                print(f"Epoch [{epoch:4d}], MAE: {mae.item():.5f}")
            if early_stop_adam.early_stop:
                print("warm adam early stopping")
                early_stop_adam.Reset()
                break
    if config['warm_bfgs_epoch'] > config['max_iter']:
        print("warm lbfgs start:") 
        for iter in range(config['warm_bfgs_epoch']//config['max_iter']):
            def closure1_wrapper():
                loss, mae = closure1(
                    model, Ixy, XY_roi, XY, RG, DG, ROI, SCALE['scale'][i]/scale_factor
                    )
                if config['epoch']%config['print_feq'] == 1:
                    epoch =  config['epoch'] 
                    print(f"Epoch [{epoch:4d}], MAE: {mae.item():.5f}")
                return loss
            loss = closure1_wrapper()
            optimizer_lbfgs.step(closure1_wrapper)
            if early_stop_lbfgs.early_stop:
                print("warm lbfgs early stopping")
                early_stop_lbfgs.Reset()
                break
            
def train_stage(i, Ixy, XY_roi, XY, RG, DG, ROI, scale_factor):
    if config['train_adam_epoch'] != 0:
        print("train adam start:")
        optimizer_adam.param_groups[0]['lr'] = config['train_lr']
        for iter in range(config['train_adam_epoch']):
            optimizer_adam.zero_grad()
            UV = model(Ixy)
            loss, mae = criterion_train(
                UV, XY_roi, XY, RG, DG, ROI, SCALE['scale'][i]/scale_factor
                )
            loss.backward()
            optimizer_adam.step()
            config['epoch'] += 1
            early_stop_adam(mae, model, optimizer_adam)
            if config['epoch']%config['print_feq'] == 1:
                epoch =  config['epoch'] 
                print(f"Epoch [{epoch:4d}], MAE: {mae.item():.5f}")
            if early_stop_adam.early_stop:
                print("train adam early stopping")
                early_stop_adam.Reset()
                break
    if config['train_bfgs_epoch'] > config['max_iter']:
        print("warm lbfgs start:") 
        for iter in range(config['warm_bfgs_epoch']//config['max_iter']):
            def closure2_wrapper():
                loss, mae = closure2(
                    model, Ixy, XY_roi, XY, RG, DG, ROI, SCALE['scale'][i]/scale_factor
                    )
                if config['epoch']%config['print_feq'] == 1:
                    epoch =  config['epoch']
                    print(f"Epoch [{epoch:4d}], MAE: {mae.item():.5f}")
                return loss
            loss= closure2_wrapper()
            optimizer_lbfgs.step(closure2_wrapper)
            if early_stop_lbfgs.early_stop:
                print("train lbfgs early stopping")
                early_stop_lbfgs.Reset()
                break

def set_early_stop_warmup(num):
    early_stop_adam.path  = f"./weights/checkpoint/example{num+1:04d}_warm_adam.pth"
    early_stop_lbfgs.path = f"./weights/checkpoint/example{num+1:04d}_warm_lbfgs.pth"
    early_stop_adam.delta = config['delta_warm_adam']
    early_stop_lbfgs.delta = config['delta_warm_lbfgs']
    optimizer_adam.param_groups[0]['lr'] = config['warm_lr']
    
def set_early_stop_train(num):
    early_stop_adam.path  = f"./weights/checkpoint/example{num:04d}_train_adam.pth"
    early_stop_lbfgs.path = f"./weights/checkpoint/example{num:04d}_train_lbfgs.pth"
    early_stop_adam.delta = config['delta_train_adam']
    early_stop_lbfgs.delta = config['delta_train_lbfgs']
    
def predict_stage(i, Ixy, XY_roi, XY, RG, DG, ROI, uv, xyuv):
    model.eval()
    UV = model(Ixy)
    loss, mae = criterion_train(UV, XY_roi, XY, RG, DG, ROI, SCALE['scale'][i])
    save_checkpoint(
        model, optimizer_adam, optimizer_lbfgs, config['epoch'], 
        mae, config['model_path']+f"model{i+1:04d}.pth"
        )
    UV[:, 0] = UV[:, 0] * SCALE['scale'][i][0] + SCALE['scale'][i][2]
    UV[:, 1] = UV[:, 1] * SCALE['scale'][i][1] + SCALE['scale'][i][3]
    
    coords = XY_roi
    U = torch.zeros_like(RG).to(device)
    V = torch.zeros_like(RG).to(device)
    y_coords, x_coords = coords[:, 0], coords[:, 1]
    U[y_coords, x_coords] = UV[:, 0]
    V[y_coords, x_coords] = UV[:, 1]
    uv[i,0,:,:] = U; uv[i,1,:,:] = V
    xyuv[i,:,0:2] = coords; xyuv[i,:,2:4] = UV
    return uv, xyuv

def pyramid_calculate(i, def_img, scale_factor_list, uv, xyuv):
    print(f"Calculate the {i+1:04d}-th deformed image start:")
    model.train()
    for scale_factor in scale_factor_list:
        RG = RGL[f'{scale_factor}']; ROI = ROIL[f'{scale_factor}']
        XY = XYL[f'{scale_factor}']; XY_roi = XY_roiL[f'{scale_factor}']
        Ixy = IxyL[f'{scale_factor}']
        DG = img_dataset.down_sample(def_img, scale_factor, device)
        print("warm up:"); set_early_stop_warmup(i)
        warm_up(i, Ixy, XY_roi, XY, RG, DG, ROI, scale_factor)
        print("train:"); set_early_stop_train(i)
        train_stage(i, Ixy, XY_roi, XY, RG, DG, ROI, scale_factor)
    uv, xyuv = predict_stage(i, Ixy, XY_roi, XY, RG, DG, ROI, uv, xyuv)
    return uv, xyuv

if __name__ == '__main__':
    
    batchframes = config['Batchframes']
    frames_list = list(range(batchframes))
    frames_list.remove(batchframes//2)
    
    rg, _, _, xy_roi, _ = img_dataset.data_collect(1, device)
    H, L = rg.shape; N = len(img_dataset)
    total = xy_roi.shape[0]
    uv = torch.zeros((N, 2, H, L))
    xyuv = torch.zeros((N, total, 4))
    BATCH = math.floor(N / batchframes)
    
    print("train start")
    
    for batch, DGlist in enumerate(train_loader):
        if batch == BATCH:
            DG = DGlist[-1]
            model.unfreeze_and_initialize()
            i = batch*batchframes + len(DGlist) - 1
            scale_factor_list = [4, 2, 1]
            pyramid_calculate(i, DG, scale_factor_list, uv, xyuv)
            print("-------------*-------------")
            for j, DG in enumerate(DGlist[:-1]):
                DG = DG
                model.freeze_layers(); # model.set_kaiming_initialization()
                i = batch*batchframes + j
                scale_factor_list = [1]
                pyramid_calculate(i, DG, scale_factor_list, uv, xyuv)
                print("-------------*-------------")
        else:
            DG = DGlist[batchframes//2]
            model.unfreeze_and_initialize()
            i = batch*batchframes + batchframes//2
            scale_factor_list = [4, 2, 1]
            pyramid_calculate(i, DG, scale_factor_list, uv, xyuv)
            print("-------------*-------------")
            for j in frames_list:
                DG = DGlist[j]
                model.freeze_layers(); # model.set_kaiming_initialization()
                i = batch*batchframes + j
                scale_factor_list = [1]
                pyramid_calculate(i, DG, scale_factor_list, uv, xyuv)
                print("-------------*-------------")
    uv = uv.cpu().detach().numpy()
    xyuv = xyuv.cpu().detach().numpy()
    to_matlab(config['data_path'], 'result', uv)
    to_txt(config['data_path'], 'result', xyuv)
        
        
        
        

